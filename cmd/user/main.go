...
import (
	...
	user "github.com/qingyggg/aufer/kitex_gen/cmd/user/userhandler"
...
func init() {
	...
	mydal.Init()
	tracer.InitJaeger(constants.UserService)
}
func main() {
	...
	addr, err := net.ResolveTCPAddr("tcp", ip+":18013")
	if err != nil {
		panic(err)
	}
	svr := user.NewServer(new(UserHandlerImpl),
		server.WithServerBasicInfo(&rpcinfo.EndpointBasicInfo{ServiceName: constants.UserService}), // server name
