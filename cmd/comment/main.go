...
func init() {
	...
	mydal.Init()
	tracer.InitJaeger(constants.CommentService)
}
func main() {
	...
	addr, err := net.ResolveTCPAddr("tcp", ip+":18012")
	if err != nil {
		panic(err)
	}
	svr := comment.NewServer(new(CommentHandlerImpl),
		server.WithServerBasicInfo(&rpcinfo.EndpointBasicInfo{ServiceName: constants.CommentService}), // server name
