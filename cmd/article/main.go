...
import (
	...
	article "github.com/qingyggg/aufer/kitex_gen/cmd/article/articlehandler"
...
func init() {
	...
	mydal.Init()
	tracer.InitJaeger(constants.ArticleService)
	go db.StartPeriodicSyncForViewCt()
}
...
func main() {
	...
	addr, err := net.ResolveTCPAddr("tcp", ip+":18010")
	if err != nil {
		panic(err)
	}
	svr := article.NewServer(new(ArticleHandlerImpl),
		server.WithServerBasicInfo(&rpcinfo.EndpointBasicInfo{ServiceName: constants.ArticleService}), // server name
